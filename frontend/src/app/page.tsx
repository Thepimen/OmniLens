...
export default function Home() {
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    if (acceptedFiles.length > 0) {
      ...
      // Reset only logic state, don't kill the file we are about to upload
  ...
  // UseEffect to trigger upload automatically when file is set after drop
  useEffect(() => {
        ...
        {!file ? (
          ...
          <div {...getRootProps()} className="flex-1 flex flex-col items-center justify-center relative w-full h-full p-6 outline-none cursor-pointer group">
            ...
            <motion.div
              ...
              className={`w-full max-w-2xl aspect-video rounded-[2rem] border backdrop-blur-xl flex flex-col items-center justify-center p-12 text-center transition-all duration-300 relative overflow-hidden ${
                isDragActive 
                  ? 'bg-indigo-500/20 border-indigo-400/50 shadow-[0_0_80px_rgba(99,102,241,0.5)]' 
            ...
            >
              ...
              <AnimatePresence>
                {isDragActive && (
                  <motion.div
                    ...
                    className="absolute inset-0 bg-gradient-to-tr from-indigo-500/30 via-transparent to-teal-500/30 animate-pulse pointer-events-none"
                  />
                )}
              </AnimatePresence>
              ...
              <div className="relative z-10 flex flex-col items-center pointer-events-none">
                <motion.div
                  ...
                  className={`w-28 h-28 mb-8 rounded-full flex items-center justify-center border transition-all duration-300 ${
                    isDragActive 
                      ? 'bg-teal-500/20 border-teal-400/50 shadow-[0_0_40px_rgba(45,212,191,0.4)]' 
                      : 'bg-gradient-to-tr from-indigo-500/20 to-teal-500/20 border-white/10 group-hover:border-indigo-400/40'
                  }`}
                >
                  <UploadCloud className={`w-14 h-14 transition-colors duration-300 ${isDragActive ? 'text-teal-300' : 'text-indigo-400'}`} />
                </motion.div>
                
                <h2 className={`text-3xl font-extrabold mb-4 tracking-tight transition-colors duration-300 ${isDragActive ? 'text-teal-300' : 'text-slate-100'}`}>
                  {isDragActive ? 'Drop it here!' : 'Drag & drop your video'}
                </h2>
                
                <p className="text-slate-400 text-lg mb-8 font-medium">or click anywhere to browse your files</p>
                
                <div className="flex gap-4 text-sm font-mono text-slate-400">
                  <span className="px-4 py-2 bg-black/40 rounded-xl border border-white/5 backdrop-blur-md font-semibold tracking-wider">MP4</span>
                  <span className="px-4 py-2 bg-black/40 rounded-xl border border-white/5 backdrop-blur-md font-semibold tracking-wider">MOV</span>
                  <span className="px-4 py-2 bg-black/40 rounded-xl border border-white/5 backdrop-blur-md font-semibold tracking-wider">Max 500MB</span>
                </div>
              </div>
            </motion.div>
          </div>
                      ...
                      {jobResult.keyframes && jobResult.keyframes.length > 0 && (
                        <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl p-5 shadow-lg">
                          ...
                          <div className="flex gap-4 overflow-x-auto pb-4 custom-scrollbar">
                            {jobResult.keyframes.map((kf: Keyframe, idx: number) => (
                              <motion.div
                                key={idx}
                ...
                {messages.length === 0 ? (
                  ...
                  <AnimatePresence>
                    {messages.map((msg, i) => (
                      ...
                      >
                        <div className={`max-w-[85%] rounded-3xl px-6 py-4 text-[15px] leading-relaxed shadow-xl ${msg.role === 'user'
